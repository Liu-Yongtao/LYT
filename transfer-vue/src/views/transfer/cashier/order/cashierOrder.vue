   ...
   <div class="app-container">
    ...
    <el-form-item label="表年份">
     ...
     <el-form-item label="平台订单号">
      ...
      <el-form-item label="话商订单号">
        <el-input v-model="formInline.outLogno" placeholder="话商订单号" style="width:180px"></el-input>
      </el-form-item>
      ...
      <el-form-item label="运营商" >
        <el-select v-model="formInline.telephoneType" placeholder="选择状态" style="width:140px">
          <el-option label="--  --" value=""></el-option>
      ...
      <el-form-item>
        <el-button type="primary" @click="getCashierOrderList(0)" size="mini">查询</el-button>
...
<!-- <el-table-column type="expand">
  <template>
        <el-table
            ...
            <el-table-column prop="urlStatus" label="产码状态" width="150" align="center">
              <template slot-scope="scope">
                <el-tag
                  :type="scope.row.urlStatus === 0 ? 'primary' : scope.row.urlStatus===1?'success':'danger'"
                  disable-transitions>{{scope.row.urlStatus === 0 ? '充值中' : scope.row.urlStatus===1?'充值成功':'充值失败'}}</el-tag>
              </template>
            </el-table-column>
            <el-table-column prop="payStatus" label="支付状态" width="150" align="center">
              <template slot-scope="scope">
                <el-tag
                  :type="scope.row.payStatus === 0 ? 'primary' : scope.row.payStatus===1?'success':'danger'"
                  disable-transitions>{{scope.row.payStatus === 0 ? '充值中' : scope.row.payStatus===1?'充值成功':'充值失败'}}</el-tag>
              </template>
            </el-table-column>
    ...
    <el-table-column prop="outLogno" label="话商订单号" width="200" align="center"></el-table-column>
    ...
    <el-table-column prop="status" label="状态" width="120" align="center">
        <template slot-scope="scope">
            <el-tag
              ...
              disable-transitions>{{scope.row.status === 0 ? '充值中' : scope.row.status===1?'充值失败':'充值成功'}}</el-tag>
        </template>
    </el-table-column>
    ...
    <el-table-column prop="endTime" label="完成时间" width="200" align="center"></el-table-column>
    ...
    <!-- 详细页 -->
  <cashier-order-details v-if="details" ref="cashierOrderDetails" />
    <!-- 分页组件 -->
    <el-pagination
...
import cashierOrderDetails from './cashier-order-details.vue'
  export default {
    inject: ['refresh'],
    data(){
      return{
    ...
    created(){
        ...
        // 获取数据
        this.getCashierOrderList(0);
        this.handleCurrentChange = this.handleCurrentChange();
    },
    methods:{
        //获取话商订单号
      getCashierOrderList(stat){
        this.loading = true;
        if(stat==0){
          this.currentPage = 1
        }else{
        ...
        request({
              url: '/transfer/getCashierOrderList',
              method: 'post',
              data:{
                cashierId:this.formInline.cashierId,
                account:this.formInline.account,
                outLogno:this.formInline.outLogno,
              ...
              this.orderList = data.cashierOrderList;
              this.loading = false;
            })
      ...
      handleDetails(data){
          ...
          this.details = true;
          this.$nextTick(()=>{
            this.$refs.cashierOrderDetails.init(data);
          })
      ...
      handleCurrentChange(){
            return this.$JsUtil.debounce((val)=>{
              ...
              this.getCashierOrderList(limit);
            },500);
