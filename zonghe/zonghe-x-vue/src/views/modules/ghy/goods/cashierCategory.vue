    ...
    <el-form :inline="true" :model="formInline" class="demo-form-inline">
     ...
     <el-form-item label="类型名称">
      ...
      <el-form-item>
        ...
        <el-button type="primary" @click="getCashierCategoryList(0)">查询</el-button>
      </el-form-item>
    </el-form>

 <el-table
    v-loading="loading"
    :data="cashierCategoryList"
    border
...
<el-button @click="updateCashierCategoryList(scope.row.id,!scope.row.status?1:0)" :type="!scope.row.status?'primary':'danger'" size="small">{{!scope.row.status?'上架':'下架'}}</el-button>
        </template>
    </el-table-column>
  </el-table>
...
<!-- 新增 or 修改 -->
  <addor-update ref="addOrUpdate" v-if="showAddOrUpdate" @getCashierCategoryList="getCashierCategoryList"></addor-update>
  </div>
   
</template>

<script>
  import AddorUpdate from './cashierCategory-add-or-update.vue';
  import Dialog from '../../../comUtil/dialog.vue';
  export default {
    inject: ['refresh'],
    data(){
      return{
        cashierCategoryList:[],          //数据
        sumCount:0,           //查询数据行数
        staLimit:0,          //起始分页
        currentPage:1,        //当前页数
    ...
    methods:{
      ...
      getCashierCategoryList(sta){
        ...
        this.$http({
              url: this.$http.adornUrl('/goods/cashierCategoryList'),
              method: 'post',
              data:{
              ...
              this.cashierCategoryList = data.cashierCategoryList;
      ...
      */
      ...
      updateCashierCategoryList(id,status){
          this.$http({
              url: this.$http.adornUrl('/goods/updateCashierCategory'),
              method: 'post',
              data:{
                id:id,
                status:status,
